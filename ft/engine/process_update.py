...
from ft.engine.layout import process_digest, read_manifest, register_project_process
...
def _copy_bundle(source: Path, destination: Path) -> None:
    """Copia um bundle de processo ignorando snapshot/backup/staging internos."""
    shutil.copytree(
        source,
        destination,
        ignore=shutil.ignore_patterns(BASE_SNAPSHOT_DIR, BACKUP_ROOT, STAGING_ROOT),
    )
...
def scan_processes(
    ...
    for name, record in sorted(processes.items()):
        ...
        if local_dir.resolve() == paths.project_process_dir(root).resolve():
            # Layout flat legado: o "dir do processo" é o catálogo inteiro —
            # qualquer varredura/merge arrastaria os processos nomeados juntos.
            states.append(ProcessDriftState(
                name=name, template_id=template_id, entrypoint=entrypoint,
                local_dir=local_dir, local_process=local_process,
                template_dir=template_dir,
                state=STATE_LEGACY_FLAT,
                detail="layout flat legado — migre com ft migrate-layout "
                       "para habilitar o update",
            ))
            continue
...
def _iter_bundle_files(directory: Path) -> dict[str, Path]:
    ...
    if not directory.is_dir():
        return files
    skip = {BASE_SNAPSHOT_DIR, BACKUP_ROOT, STAGING_ROOT}
...
def apply_update(
    ...
    try:
        ...
        register_project_process(
            ...
            process_path=state.local_process,
