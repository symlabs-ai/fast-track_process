...
- [ ] C16: Toda capacidade P0 declarada como entregue é alcançável a partir da primeira abertura ou de uma superfície principal usando somente menus, abas ou controles visíveis da UI de produção. Rota/URL interna direta, deep link de debug, catálogo técnico, montagem isolada de componente e hook test-only não contam como prova. Capacidades condicionadas por papel, coorte ou flag possuem evidência positiva para identidade elegível e negativa para identidade comum.
- [ ] C17: Cada target obrigatório da matriz de plataforma é validado no ambiente declarado com o candidato corrente. Em Android/iOS físico, simulador ou emulador não substitui o aparelho; em web, viewport não substitui navegador real; em desktop, outro sistema operacional não substitui o alvo. Evidências cobrem instalação/abertura quando aplicável, navegação, acessibilidade, estado, insets/safe areas e comparação visual sem reutilizar uma prova entre targets.
- [ ] C18: Toda tela, página ou janela da UI de produção exibe uma marca d'água discreta, legível e não interativa com o identificador exato do mockup correspondente (por exemplo, `S01`, `S02`). A marca pertence ao produto renderizado, não à ferramenta de captura. O inventário tela→mockup deve estar completo em Android, iOS, web e desktop; tela sem mockup atribuído ou screenshot real que comprove a marca reprova a validação.
