...
def project(tmp_path):
    ...
    shutil.copytree(
        REPO_ROOT / "templates" / "feature-fast", process_dir / "feature-fast"
    )
        ...
        yaml.safe_dump(
            {
                ...
                "processes": {
                    "feature-fast": {
                        "path": ".ft/process/feature-fast/process.yml",
...
def fake_engine(tmp_path):
    ...
    shutil.copytree(
        REPO_ROOT / "templates" / "evolve_process", templates / "evolve_process"
    )
    shutil.copytree(
        REPO_ROOT / "templates" / "feature-fast", templates / "feature-fast"
    )
    return engine


def _active_cycle(
    ft_home_dir: Path, project_root: Path, name: str = "cycle-03"
) -> Path:
    ...
    (cycle / "docs").mkdir()
    (cycle / "docs" / "retro.md").write_text(
        "# Retro\nnode X falhou 3x\n", encoding="utf-8"
    )
    return cycle
...
def test_resolve_targets_global_via_manifesto(project, fake_engine):
    ...
    assert set(targets.global_dirs) == {"feature-fast"}
    assert (
        targets.global_dirs["feature-fast"]
        == fake_engine / "templates" / "feature-fast"
    )
    assert targets.labels == ["project", "global:feature-fast"]
...
def _run_evolve_apply(
    workspace: evolve.EvolveWorkspace,
...
def test_prepare_workspace_monta_staging_e_contexto(ft_home, project, fake_engine):
    ...
    assert (ws.staged_project_dir / "feature-fast" / "process.yml").is_file()
    assert (ws.staged_global_dir("feature-fast") / "process.yml").is_file()
    assert (ws.context_dir / "cycle" / "docs" / "retro.md").is_file()
    assert (ws.context_dir / "cycle_state.yml").is_file()
    assert "focar em retries" in (ws.context_dir / "directive.md").read_text(
        encoding="utf-8"
    )
    manifest = (ws.context_dir / "targets.md").read_text(encoding="utf-8")
    assert "targets/project/" in manifest
    assert "targets/global/feature-fast/" in manifest

    meta = yaml.safe_load((ws.root / "workspace.yml").read_text(encoding="utf-8"))
    assert meta["targets"] == ["project", "global:feature-fast"]
...
def test_validate_staged_detecta_yaml_quebrado(ft_home, project, fake_engine):
    ws = _workspace(project, fake_engine, ft_home, include_global=False)
    (ws.staged_project_dir / "feature-fast" / "process.yml").write_text(
...
def test_validate_staged_exige_process_yml(ft_home, project, fake_engine):
    ws = _workspace(project, fake_engine, ft_home, include_global=False)
    (ws.staged_project_dir / "feature-fast" / "process.yml").unlink()
    errors = evolve.validate_staged(ws)
...
def test_validate_staged_global_pristine(ft_home, project, fake_engine):
    ...
    (ws.staged_global_dir("feature-fast") / "engine_state.yml").write_text(
        "x: 1\n", encoding="utf-8"
    )
    errors = evolve.validate_staged(ws)
    assert any(
        "global:feature-fast" in error and "estado de execução" in error
        for error in errors
    )
...
def test_diff_e_apply_espelham_staging(ft_home, project, fake_engine):
    ...
    staged_process = ws.staged_project_dir / "feature-fast" / "process.yml"
    staged_process.write_text(
        staged_process.read_text(encoding="utf-8") + "\n# melhoria EV-01\n",
        encoding="utf-8",
    )
    (ws.staged_project_dir / "feature-fast" / "scripts" / "novo.sh").write_text(
        "#!/bin/sh\n", encoding="utf-8"
    )
    staged_readme = ws.staged_global_dir("feature-fast") / "README.md"
    ...
    assert ("project", "modified", "feature-fast/process.yml") in summary
    assert ("project", "added", "feature-fast/scripts/novo.sh") in summary
    assert ("global:feature-fast", "removed", "README.md") in summary
    assert len(changes) == 3

    evolve.apply_staged(ws, changes)
    real_process = project / ".ft" / "process" / "feature-fast" / "process.yml"
    assert "# melhoria EV-01" in real_process.read_text(encoding="utf-8")
    assert (
        project / ".ft" / "process" / "feature-fast" / "scripts" / "novo.sh"
    ).is_file()
    assert not (fake_engine / "templates" / "feature-fast" / "README.md").exists()
...
def test_apply_project_target_respects_exclusive_startup_barrier(
    ...
    process_relative = Path(".ft/process/feature-fast/process.yml")
    ...
    try:
        ...
        real_process = project_processes / "feature-fast" / "process.yml"
...
class _FakeRunner:
    ...
    def run(self, mode):
        assert mode == "mvp"
        staged = self.workspace / "targets" / "project" / "feature-fast" / "process.yml"
        staged.write_text(
        ...
        (report_dir / "evolution-report.md").write_text(
            "# Evolution Report\n", encoding="utf-8"
        )
...
def test_cmd_evolve_aplica_no_projeto(
    ft_home, project, fake_engine, cli_engine, monkeypatch, capsys
):
    monkeypatch.chdir(project)
    cli_main.cmd_evolve(_evolve_args())
    out = capsys.readouterr().out
    assert "EV-01 aplicado" in (
...
def test_cmd_evolve_dry_run_nao_aplica(
    ft_home, project, fake_engine, cli_engine, monkeypatch, capsys
):
    monkeypatch.chdir(project)
    before = (project / ".ft" / "process" / "feature-fast" / "process.yml").read_text(
        encoding="utf-8"
    )
    cli_main.cmd_evolve(_evolve_args(dry_run=True))
    out = capsys.readouterr().out
    assert "--dry-run" in out
    assert (project / ".ft" / "process" / "feature-fast" / "process.yml").read_text(
        encoding="utf-8"
    ) == before


def test_cmd_evolve_exige_alvo(
    ft_home, project, fake_engine, cli_engine, monkeypatch, capsys
):
    monkeypatch.chdir(project)
...
def test_cmd_evolve_global_exige_git_no_engine(
    ft_home, project, fake_engine, cli_engine, monkeypatch, capsys
):
    monkeypatch.chdir(project)
...
def test_cmd_evolve_template_de_outro_entrypoint(
    ft_home, project, fake_engine, cli_engine, monkeypatch, capsys
):
    monkeypatch.chdir(project)
    with pytest.raises(SystemExit):
        cli_main.cmd_evolve(_evolve_args(template="feature-fast"))
