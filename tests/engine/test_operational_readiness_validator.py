...
SCRIPT = ROOT / "templates" / "mvp-builder-fast" / "scripts" / "validate_operational_readiness.py"
...
def _fixture(root: Path, *, css: str = "body { font-size: 12px; }\n", source: str = "print('production')\n") -> tuple[Path, Path]:
    (root / "src").mkdir()
    ...
    report = {
        ...
        "runtime": {
            "mode": "production", "clean_start": True, "restart_verified": True,
            "demo_seed_enabled": False, "synthetic_runtime_records": False,
            "mock_providers_enabled": False, "persistence": "durable",
        },
        "ui": {
            "applicable": True, "minimum_observed_font_px": 12,
            "zoom_percent": 100, "evidence": ["docs/evidence/operational/OP-001.txt"],
        },
        "scan": {"production_paths": ["src"], "prohibited_hits": []},
        "results": [{
            "ref": "PB-001", "result": "PASS", "journeys": ["OP-001"],
            "evidence": ["docs/evidence/operational/OP-001.txt"],
        }],
        "journeys": [{
            "id": "OP-001", "refs": ["PB-001"], "result": "PASS",
            "navigation_mode": "production_ui",
            "data_origin": "created_via_public_interface",
            "canary": {
                "created_value": "canary-123", "persisted_value": "canary-123",
    ...
    contract = {
        "definition_of_done": {"required_gates": [{
            "id": "operational-real-data", "path": "docs/operational-readiness.yml",
            "field": "verdict", "equals": "APPROVED",
        }]},
    }
...
def test_rejects_demo_runtime_and_illegible_production_text(
    tmp_path: Path, css: str, source: str, message: str,
) -> None:
