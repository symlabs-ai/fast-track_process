...
def test_legacy_flat_process_is_not_actionable(project, fake_engine):
    states = pu.scan_processes(project, _templates_root(fake_engine), process_name="base")
    assert states and states[0].state == pu.STATE_LEGACY_FLAT
    assert pu.STATE_LEGACY_FLAT not in pu.ACTIONABLE_STATES
...
def test_merge_conflict_keeps_diff3_markers(project, fake_engine):
    ...
    local_process.write_text(
        local_process.read_text(encoding="utf-8").replace(
            'version: "1.0.0"', 'version: "1.5.0-fork"'
        ),
        encoding="utf-8",
    )
...
def test_apply_update_rolls_back_on_failure(project, fake_engine):
    ...
    with patch.object(pu, "register_project_process", side_effect=RuntimeError("boom")):
